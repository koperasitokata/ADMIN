...
function getDashboardData(payload) {
  ...
  if (role === "ADMIN") {
    ...
    // MUTASI HARIAN (Gabungan semua transaksi)
    const mutations = [];
    ...
    // Robust sheet selection for PEMASUKAN
    const pemasukanSheet = ss.getSheetByName("PEMASUKAN") || ss.getSheets().find(s => s.getName().trim().toUpperCase() === "PEMASUKAN");
    const pemasukan = getSheetData(pemasukanSheet);
    ...
    pemasukan.forEach(d => mutations.push({...d, tipe: 'Pemasukan', nominal: d.jumlah, ket: d.keterangan || 'Admin Cair 5%'}));

    results.mutasi = mutations;
    results.pemasukan_list = pemasukan;
