...
// Initialize DB if not exists (Vercel has a read-only filesystem, but we can try to read/write in /tmp if needed, 
// but for now we keep it in the root for mock purposes)
function getDB() {
  try {
...
function saveDB(data: any) {
  try {
    // Note: This will not persist on Vercel between function calls
...
app.post("/api", async (req, res) => {
  console.log(`[API] Received request: ${req.body?.action}`);
  const remoteUrl = process.env.VITE_API_URL;
  
  // Proxy Mode to Google Apps Script
  if (remoteUrl && remoteUrl.startsWith('http')) {
    try {
      ...
      try {
        ...
        return res.json({ 
          success: false, 
          message: "Gagal memproses data dari Google Script.",
  ...
  // Local Mock Mode (Limited persistence on Vercel)
  try {
    ...
    switch (action) {
      ...
      default:
        return res.json({ success: false, message: "Action not implemented: " + action });
    }
  } catch (err: any) {
